...
export function ManageExpenseCategoriesDialog({ isOpen, onOpenChange }: ManageExpenseCategoriesDialogProps) {
  const { toast } = useToast();
  const { expenseCategories, setExpenseCategories } = useExpenses();
  ...
  const handleAddCategory = () => {
    ...
    if (expenseCategories.some(c => c.name.toLowerCase() === newCategoryName.trim().toLowerCase())) {
        toast({ variant: 'destructive', title: 'Category already exists.' });
        return;
    }
    const newCategory: ExpenseCategory = {
      id: `exp_cat_${new Date().getTime()}`,
  ...
  const handleUpdateCategory = () => {
    if (!editingCategoryId) return;
     ...
     if (expenseCategories.some(c => c.id !== editingCategoryId && c.name.toLowerCase() === editingCategoryName.trim().toLowerCase())) {
        toast({ variant: 'destructive', title: 'Category name already exists.' });
        return;
    }
    setExpenseCategories(prev => prev.map(c => c.id === editingCategoryId ? { ...c, name: editingCategoryName.trim() } : c));
