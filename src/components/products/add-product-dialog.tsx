...
interface AddProductDialogProps {
  ...
  categories: Category[];
  onProductAdded: (product: Omit<ProductFormValues, 'image'> & { image: File | string }) => void;
}

export function AddProductDialog({ isOpen, onOpenChange, categories, onProductAdded }: AddProductDialogProps) {
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    // Ensure image is provided as File or string
    const productData = {
      ...data,
  ...
  const takePicture = () => {
    if (videoRef.current) {
      ...
      stopCamera();
      setImageSource('upload'); // Switch back to upload tab to show preview
    }
  };
                    ...
                    <TabsContent value="upload" className="mt-2">
                       {imagePreview ? (
                           ...
                           <div className="flex items-center justify-center w-full">
                                <Label htmlFor="product-image-upload" className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer hover:bg-muted">
                                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                        <Upload className="w-8 h-8 mb-4 text-muted-foreground" />
                                        <p className="mb-2 text-sm text-muted-foreground"><span className="font-semibold">Click to upload</span> or drag and drop</p>
                                        <p className="text-xs text-muted-foreground">PNG, JPG, GIF up to 10MB</p>
                                    </div>
                                    <Input id="product-image-upload" type="file" className="hidden" onChange={handleFileChange} accept="image/*" />
                                </Label>
                            </div> 
                       )}
                    </TabsContent>
